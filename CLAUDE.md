...
- **`fatura.py`** – Values *derived* from an invoice row: `kaynak(satir, bilinmeyen="")` and `dosya_adi(satir, bos="")`. The row itself is still a plain `dict`; only the derivations live here, because the same two lines were copied into `worker`, `api`, `excel_utils` and `ozet` (and the `ozet` copy had already drifted to a `"Bilinmiyor"` default — hence the `bilinmeyen` argument).
